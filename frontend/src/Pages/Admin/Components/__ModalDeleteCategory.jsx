...
export const ModalDeleteCategory = ({ item, getCategoryAPI }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await axios.get("sanctum/csrf-cookie").then(() => {
      axios.post(`api/category/delete/${item.id}`).then((res) => {
        setLoadSubmit(false);
        if (res.data.status === 201) return toast.warning("Server error");
        toast.success("Berhasil dihapus");
        document.getElementById(`my-category-delete${item.id}`).click();
        getCategoryAPI();
      });
    });
  };
  return (
    <>
      <input
        type="checkbox"
        id={`my-category-delete${item.id}`}
        className="modal-toggle"
      />
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg text-center">Hapus Kategori!</h3>
