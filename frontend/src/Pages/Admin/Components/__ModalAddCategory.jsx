...
export const ModalAddCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoadSubmit(true);
    const data = {
      category_name: formInput.nama_kategori,
    };
    await axios.get("sanctum/csrf-cookie").then(() => {
      axios.post("api/category/store", data).then((res) => {
        setLoadSubmit(false);
        if (res.data.status === 201) return toast.warning("Server error");
        if (res.data.status === 202)
          return toast.warning("Nama kategori sudah ada");
        toast.success("Berhasil ditambahkan");
        setFormInput({
          nama_kategori: "",
        });
        document.getElementById("my-modal-category").click();
      });
    });
  };
  return (
    <>
      <input type="checkbox" id="my-modal-category" className="modal-toggle" />
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg">Tambahkan Kategori Baru!</h3>
          <form action="">
            <p className="py-4">
