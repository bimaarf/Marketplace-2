...
export const ModalAddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    pushServer.append("special_price", formInput.special_price);
    ...
    await axios.get("sanctum/csrf-cookie").then(() => {
      axios.post("api/product/store", data, config).then((res) => {
        setLoadSubmit(false);
        if (res.data.status === 201) return toast.warning("Server error");
        if (res.data.status === 203)
          return toast.warning("Nama produk sudah ada");
        if (res.data.status === 202)
          return toast.warning("Masukkan data dengan benar");
        toast.success("Produk berhasil ditambahkan");
        document.getElementById(`my-modal-product`).click();
      });
    });
  };
  return (
    <>
      <input type="checkbox" id="my-modal-product" className="modal-toggle" />
      <div className="modal">
        <div className="modal-box w-11/12 max-w-5xl">
          ...
          <div className="py-4">
            ...
            <div className="md:flex justify-center items-start gap-2">
              ...
              <div className="py-2 w-1/2">
                ...
                <div className="flex justify-start gap-2 items-center">
                  ...
                  <input
                    ...
                    name="special_price"
                    className="w-full outline-none py-2 pl-8 pr-2 focus:border-green-500 border text-sm md:text-md"
                    placeholder="Kosongkan jika tak ada promo"
                  />
                </div>
              </div>
            </div>
